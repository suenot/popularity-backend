...
SELECT DISTINCT ON (channel_id)
    channel_id, ts, followers, posts_count, total_likes, total_views, total_comments
FROM channel_snapshots
ORDER BY channel_id, ts DESC;

-- Helper: most-recent snapshot at or before (now - N days). Returns NULL row if absent.
CREATE OR REPLACE FUNCTION snapshot_at(ch BIGINT, days_back INT)
RETURNS channel_snapshots
LANGUAGE sql
STABLE
AS $$
    SELECT *
...
-- Per-channel KPI row: current values, deltas over 7/30/90/365d, CAGR, velocity.
